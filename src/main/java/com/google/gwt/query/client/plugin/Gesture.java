...
public class Gesture extends Events {
  ...
  static {
    ...
    $.hasGestures = Window.Location.getParameter("mobile") != null ||
        Window.Navigator.getUserAgent().matches(".*(iPad|iPhone|iPod|Mobile Safari).*");
    ...
    ThresholdShake freezeShakes = $.jGestures.defaults().thresholdShake();
    freezeShakes.requiresShakes(10).freezeShakes(100);
    freezeShakes.frontback().sensitivity(10);
    freezeShakes.leftright().sensitivity(10);
    freezeShakes.updown().sensitivity(10);
  ...
  static Function _onOrientationchange = new Function() {
    ...
    public boolean f(Event event_) {
      if (_window.orientation() != _lastOrientiation) {
        ...
        $(window).trigger("orientationchange", _createOptions(GQ.create(OptArgs.class).description("orientiationchange:" +  _aDict[(_window.orientation()/90) + 1] + _window.orientation())));
      }
      return false;
    };
  };
...
//        event_.currentTarget.removeEventListener('mouseup', _onTouchend, false);
      ...
      // trigger events for all bound pseudo events on this element
      for (String _sType : _oDatajQueryGestures.keys()) {
        if (!$.special.containsValue(_sType)) continue;
